...
export default function UserLogin() {
  usePageMeta({
    title: "Sign In",
    description: "Sign in to your SPMart account to manage orders, addresses and the wishlist.",
    noIndex: true,
  });
  ...
  // Priority: ?redirect= URL param  →  router state.from  →  home
  const redirectTarget =
    searchParams.get("redirect") || location.state?.from || "/";

  // If a logged-in user lands on /login (e.g. by clicking a guarded link),
  // bounce them straight to where they were going.
  useEffect(() => {
    if (!authLoading && user) {
      navigate(redirectTarget, { replace: true });
    }
  }, [authLoading, user, redirectTarget, navigate]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gradient-to-br from-violet-50 via-white to-fuchsia-50">
      {/* Redirect notice — shown when user came here via a protected route */}
      {searchParams.get("redirect") && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-full bg-violet-600 text-white text-xs font-bold shadow-lg flex items-center gap-2">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.4}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 11c0-1.1.9-2 2-2s2 .9 2 2-2 2-2 2v1m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 rounded-xl bg-gray-50 dark:bg-white/[0.06] border border-gray-200 dark:border-white/[0.1] text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/20 transition-all"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-11 py-3 rounded-xl bg-gray-50 dark:bg-white/[0.06] border border-gray-200 dark:border-white/[0.1] text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/20 transition-all"
                />
                <button
                  type="button"
